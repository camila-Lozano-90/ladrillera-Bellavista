...
class SegundaDespachoModel
{
    ...
    {   //die(var_dump($data));
        $this->db->query("INSERT INTO segundadespacho(referencia, unidades, fecha) 
    ...
    {   //die(var_dump($data['referencia']));
        $this->db->query("SELECT SUM(segunda) FROM controldiariodespachosfooter WHERE referencia = :referencia;");
        $this->db->bind(':referencia', $data['referencia']);
        $resulset = $this->db->getOne();
        $res = json_encode($resulset->{'SUM(segunda)'});
        $res1 = trim($res,'"');
        //die($res1);
        return $res1;
    }
    ...
    {
        $this->db->query("SELECT idCrtlDespFoot FROM controldiariodespachosfooter WHERE referencia = :referencia AND segunda > 0;");
        $this->db->bind(':referencia', $data['referencia']);
        $resulset = $this->db->getOne();
